...
func spawn_enemies_random():

	var count = randi() % 3 + 1  
	

	var alignments = ["left", "center", "right"]
	var alignment = alignments[randi() % alignments.size()]
	...
	for x in range(count):
		...
		match alignment:
			"center":
				pos = Vector2(120 + x * 40, 50 + abs(x - 1) * 20)
			"right":
				pos = Vector2(80 + (x - 1) * 40, 50 + abs(x - 1) * 20)
			"left":
				pos = Vector2(160 + (x - 1) * 40, 50 + abs(x - 1) * 20)
			_:
				pos = Vector2(120 + x * 40, 50 + abs(x - 1) * 20)
